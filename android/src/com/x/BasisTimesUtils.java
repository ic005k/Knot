...
public class BasisTimesUtils {
     ...
     */
    public static BasisTimesUtils showDatePickerDialog(
        Context context,
        boolean themeLight,
        String title,
        int year,
            ...
            new DatePickerDialog.OnDateSetListener() {
                ...
                public void onDateSet(
                    ...
                    if (onDateTimePickerListener != null) {
                        onDateTimePickerListener.onConfirm(
                            year,
                            month,
                            dayOfMonth
                        );
    ...
    /**
     * 隐藏月, 只显示年（配合 setDayGone() 实现仅显示年）
     */
    public void setMonthGone() {
        setSpecialDatePicker(3); // 新增 state=3 对应隐藏月的逻辑
    }
    ...
    private void setSpecialDatePicker(int state) {
        try {
            ...
            NumberPicker view0 = (NumberPicker) (
                (ViewGroup) ((ViewGroup) dp.getChildAt(0)).getChildAt(0)
            ).getChildAt(0); //获取最前一位的宽度
            NumberPicker view1 = (NumberPicker) (
                (ViewGroup) ((ViewGroup) dp.getChildAt(0)).getChildAt(0)
            ).getChildAt(1); //获取中间一位的宽度
            NumberPicker view2 = (NumberPicker) (
                (ViewGroup) ((ViewGroup) dp.getChildAt(0)).getChildAt(0)
            ).getChildAt(2); //获取最后一位的宽度
     ...
     * 显示时间选择器
     */
    public static void showTimerPickerDialog(
        Context context,
        boolean themeLight,
        String title,
        int hourOfDay,
