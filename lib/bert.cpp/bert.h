...
// ========== 新增词表查询API（适配WordPieceTokenizer） ==========
// 获取词表总大小
BERT_API int32_t bert_get_vocab_size(struct bert_ctx* ctx);

// 根据id获取对应token字符串
BERT_API const char* bert_get_vocab_token(struct bert_ctx* ctx, bert_vocab_id idx);

// 根据token字符串反向查询id，不存在返回-1
BERT_API bert_vocab_id bert_get_token_id(struct bert_ctx* ctx, const char* token_str);
// ==============================================================
