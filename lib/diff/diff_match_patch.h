...
// --- Potential Fix for Macro Conflicts ---
// Undefine common macros that might clash with enum values
#ifdef DELETE
#undef DELETE
#endif
#ifdef INSERT
...
// Consider using enum class for better scoping (C++11)
// enum class Operation { DELETE, INSERT, EQUAL };
// But if sticking to plain enum, be wary of name clashes
enum Operation {
  DELETE_OP,
  INSERT_OP,
  EQUAL_OP
...
class Diff {
 public:
  Operation operation;
  // One of: INSERT_OP, DELETE_OP or EQUAL_OP.
  QString text;
  // The text associated with this diff operation.

  /**
   * Constructor.  Initializes the diff with the provided values.
   * @param operation One of INSERT_OP, DELETE_OP or EQUAL_OP.
   * @param text The text being applied.
   */
  Diff(Operation _operation, const QString &_text);
  Diff();
  // Remove 'inline' here if definition is in .cpp, or provide inline definition
  // here
  bool isNull() const;
...
class diff_match_patch {
 ...
 public:
   ...
   * @return Substring.
   */
  QString diff_prettyHtml_old(const QList<Diff> &diffs);
  private:
