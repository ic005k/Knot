...
// #include <QtCore>
#include <QChar>                 // 字符操作
#include <QList>                 // 用于存储Diff、Patch等列表
#include <QMap>                  // 用于字符映射（如match_alphabet函数）
#include <QMutableListIterator>  // 列表迭代器（修改列表元素）
#include <QPair>                 // 键值对（patch_apply返回值）
#include <QRegExp>               // 正则表达式（BLANKLINEEND等定义）
#include <QString>               // 用于字符串处理（大量使用）
#include <QStringList>           // 字符串列表（diff_linesToChars中的lineArray）
#include <QUrl>                  // URL编码（QUrl::toPercentEncoding等）
#include <QVariant>              // 变体类型（diff_linesToChars返回值）
#include <QVector>               // 动态数组（patch_apply返回的结果向量）
...
QList<Diff> diff_match_patch::diff_main(const QString &text1,
  ...
  if (text1 == text2) {
    if (!text1.isEmpty()) {
      diffs.append(Diff(EQUAL_OP, text1));
    }
    return diffs;
  }
  ...
  if (!commonprefix.isEmpty()) {
    diffs.prepend(Diff(EQUAL_OP, commonprefix));
  }
  if (!commonsuffix.isEmpty()) {
    diffs.append(Diff(EQUAL_OP, commonsuffix));
  }
...
QList<Diff> diff_match_patch::diff_compute(QString text1, QString text2,
  ...
  if (text1.isEmpty()) {
    ...
    diffs.append(Diff(INSERT_OP, text2));
    return diffs;
  }

  if (text2.isEmpty()) {
    // Just delete some text (speedup).
    diffs.append(Diff(DELETE_OP, text1));
    return diffs;
  }

  {
    ...
    if (i != -1) {
      ...
      const Operation op =
          (text1.length() > text2.length()) ? DELETE_OP : INSERT_OP;
      diffs.append(Diff(op, longtext.left(i)));
      diffs.append(Diff(EQUAL_OP, shorttext));
  ...
  if (hm.count() > 0) {
    ...
    // Merge the results.
    diffs = diffs_a;
    diffs.append(Diff(EQUAL_OP, mid_common));
    diffs += diffs_b;
    return diffs;
  }
...
void diff_match_patch::diff_cleanupSemantic(QList<Diff> &diffs) {
  ...
  Diff *thisDiff = pointer.hasNext() ? &pointer.next() : NULL;
  while (thisDiff != NULL) {
    if (thisDiff->operation == EQUAL_OP) {
      // Equality found.
      ...
      if (thisDiff->operation == INSERT_OP) {
      ...
      if (!lastequality.isNull() &&
        ...
        pointer.setValue(Diff(DELETE_OP, lastequality));
        // Insert a corresponding an insert.
        pointer.insert(Diff(INSERT_OP, lastequality));
  ...
  while (thisDiff != NULL) {
    if (prevDiff->operation == DELETE_OP && thisDiff->operation == INSERT_OP) {
      ...
      if (overlap_length1 >= overlap_length2) {
        if (overlap_length1 >= deletion.length() / 2.0 ||
          ...
          pointer.previous();
          pointer.insert(Diff(EQUAL_OP, insertion.left(overlap_length1)));
        ...
        if (overlap_length2 >= deletion.length() / 2.0 ||
          ...
          pointer.previous();
          pointer.insert(Diff(EQUAL_OP, deletion.left(overlap_length2)));
          prevDiff->operation = INSERT_OP;
          prevDiff->text = insertion.left(insertion.length() - overlap_length2);
          thisDiff->operation = DELETE_OP;
...
void diff_match_patch::diff_cleanupSemanticLossless(QList<Diff> &diffs) {
  ...
  while (nextDiff != NULL) {
    if (prevDiff->operation == EQUAL_OP && nextDiff->operation == EQUAL_OP) {
...
void diff_match_patch::diff_cleanupEfficiency(QList<Diff> &diffs) {
  ...
  while (thisDiff != NULL) {
    if (thisDiff->operation == EQUAL_OP) {
      ...
      // An insertion or deletion.
      if (thisDiff->operation == DELETE_OP) {
        post_del = true;
      } else {
        post_ins = true;
      }
...
void diff_match_patch::diff_cleanupMerge(QList<Diff> &diffs) {
  diffs.append(Diff(EQUAL_OP, ""));  // Add a dummy entry at the end.
  QMutableListIterator<Diff> pointer(diffs);
  int count_delete = 0;
  int count_insert = 0;
  QString text_delete = "";
      ...
      case EQUAL_OP:
        if (count_delete + count_insert > 1) {
          ...
          if (both_types) {
            ...
            if (commonlength != 0) {
              if (pointer.hasPrevious()) {
                ...
                pointer.insert(Diff(EQUAL_OP, text_insert.left(commonlength)));
              }
          ...
          if (!text_delete.isEmpty()) {
            pointer.insert(Diff(DELETE_OP, text_delete));
          }
          if (!text_insert.isEmpty()) {
            pointer.insert(Diff(INSERT_OP, text_insert));
          }
...
int diff_match_patch::diff_xIndex(const QList<Diff> &diffs, int loc) {
  ...
  Diff lastDiff;
  foreach (Diff aDiff, diffs) {
    if (aDiff.operation != INSERT_OP) {
      // Equality or deletion.
      chars1 += aDiff.text.length();
    }
    if (aDiff.operation != DELETE_OP) {
  ...
  if (lastDiff.operation == DELETE_OP) {
...
QString diff_match_patch::diff_prettyHtml_old(const QList<Diff> &diffs) {
  QString html;
  QString text;
  foreach (Diff aDiff, diffs) {
    text = aDiff.text;
  ...
  foreach (QString token, tokens) {
    ...
    switch (token[0].toLatin1()) {
      case '+':
        ...
        diffs.append(Diff(INSERT_OP, param));
        break;
      case '-':
        // Fall through.
      case '=': {
        ...
        pointer += n;
        if (token[0] == QChar('=')) {
          diffs.append(Diff(EQUAL_OP, text));
        } else {
          diffs.append(Diff(DELETE_OP, text));
        }
        break;
      }
      default:
...
int diff_match_patch::match_bitap(const QString &text, const QString &pattern,
  ...
  for (int d = 0; d < pattern.length(); d++) {
    ...
    // 将 text.length() 显式转换为 int，与 loc + bin_mid（int类型）匹配
    int finish = std::min(loc + bin_mid, static_cast<int>(text.length())) +
...
void diff_match_patch::patch_addContext(Patch &patch, const QString &text) {
  ...
  while (text.indexOf(pattern) != text.lastIndexOf(pattern) &&
    ...
    pattern =
        safeMid(text, std::max(0, patch.start2 - padding),
                // 显式将text.length()转换为int，与另一个参数（int类型）匹配
                std::min(static_cast<int>(text.length()),
  ...
  if (!prefix.isEmpty()) {
    patch.diffs.prepend(Diff(EQUAL_OP, prefix));
  }
  // Add the suffix.
  QString suffix = safeMid(
      ...
      // 显式转换两个参数为int，确保类型一致
      std::min(static_cast<int>(text.length()),
               static_cast<int>(patch.start2 + patch.length1 + padding)) -
          (patch.start2 + patch.length1));
  if (!suffix.isEmpty()) {
    patch.diffs.append(Diff(EQUAL_OP, suffix));
  }
...
QList<Patch> diff_match_patch::patch_make(const QString &text1,
  ...
  foreach (Diff aDiff, diffs) {
    if (patch.diffs.isEmpty() && aDiff.operation != EQUAL_OP) {
...
QString diff_match_patch::patch_addPadding(QList<Patch> &patches) {
  ...
  if (firstPatchDiffs.empty() ||
      firstPatchDiffs.first().operation != EQUAL_OP) {
    // Add nullPadding equality.
    firstPatchDiffs.prepend(Diff(EQUAL_OP, nullPadding));
  ...
  if (lastPatchDiffs.empty() || lastPatchDiffs.last().operation != EQUAL_OP) {
    // Add nullPadding equality.
    lastPatchDiffs.append(Diff(EQUAL_OP, nullPadding));
    ...
    while (!bigpatch.diffs.isEmpty()) {
      ...
      while (!bigpatch.diffs.isEmpty() &&
        ...
        if (diff_type == INSERT_OP) {
          ...
          empty = false;
        } else if (diff_type == DELETE_OP && patch.diffs.size() == 1 &&
                   patch.diffs.front().operation == EQUAL_OP &&
          ...
          diff_text = diff_text.left(std::min(
              static_cast<int>(diff_text.length()),
              static_cast<int>(patch_size - patch.length1 - Patch_Margin)));
          patch.length1 += diff_text.length();
          start1 += diff_text.length();
          if (diff_type == EQUAL_OP) {
            patch.length2 += diff_text.length();
            start2 += diff_text.length();
          } else {
            empty = false;
      ...
      if (!postcontext.isEmpty()) {
        ...
        if (!patch.diffs.isEmpty() &&
            patch.diffs.back().operation == EQUAL_OP) {
          patch.diffs.back().text += postcontext;
        } else {
          patch.diffs.append(Diff(EQUAL_OP, postcontext));
        }
      }
      if (!empty) {
...
QList<Patch> diff_match_patch::patch_fromText(const QString &textline) {
  ...
  while (!text.isEmpty()) {
    ...
    while (!text.isEmpty()) {
      ...
      if (sign == '-') {
        // Deletion.
        patch.diffs.append(Diff(DELETE_OP, line));
      } else if (sign == '+') {
        // Insertion.
        patch.diffs.append(Diff(INSERT_OP, line));
      } else if (sign == ' ') {
        // Minor equality.
        patch.diffs.append(Diff(EQUAL_OP, line));
      } else if (sign == '@') {
        // Start of next patch.
        break;
      } else {
