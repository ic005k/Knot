...
void ggml_quantize_mat_q8_0_4x1_generic(const float* GGML_RESTRICT x,
...
void ggml_quantize_mat_q8_K_4x1_generic(const float* GGML_RESTRICT x,
                                        void* GGML_RESTRICT vy, int64_t k) {
  assert(QK_K == 256);
  assert(k % QK_K == 0);
  const int nb = k / QK_K;
...
void ggml_quantize_mat_q8_K_4x4_generic(const float* GGML_RESTRICT x,
  ...
  for (int i = 0; i < nb; i++) {
    ...
    // Quants values are interleaved in sequence of four bytes from
...
void ggml_quantize_mat_q8_K_4x8_generic(const float* GGML_RESTRICT x,
                                        void* GGML_RESTRICT vy, int64_t k) {
  assert(QK_K == 256);
  assert(k % QK_K == 0);
  const int nb = k / QK_K;
...
template <>
void ggml_quantize_mat_t<4, GGML_TYPE_Q8_0>(const float* GGML_RESTRICT x,
...
template <>
void ggml_quantize_mat_t<8, GGML_TYPE_Q8_0>(const float* GGML_RESTRICT x,
                                            void* GGML_RESTRICT vy,
                                            int64_t nrow, int64_t n_per_row) {
  assert(nrow == 4);
  UNUSED(nrow);
...
template <>
void ggml_quantize_mat_t<4, GGML_TYPE_Q8_K>(const float* GGML_RESTRICT x,
                                            void* GGML_RESTRICT vy,
                                            int64_t nrow, int64_t n_per_row) {
  assert(nrow == 4);
  UNUSED(nrow);
...
#if defined __riscv_zvfh
template <>
void ggml_quantize_mat_t<1, GGML_TYPE_Q8_0>(const float* GGML_RESTRICT x,
                                            void* GGML_RESTRICT vy,
                                            int64_t nrow, int64_t n_per_row) {
  assert(nrow == 4);
  UNUSED(nrow);
...
template <>
void ggml_quantize_mat_t<1, GGML_TYPE_Q8_K>(const float* GGML_RESTRICT x,
                                            void* GGML_RESTRICT vy,
                                            int64_t nrow, int64_t n_per_row) {
  assert(nrow == 4);
  UNUSED(nrow);
...
static void ggml_gemv_q6_K_NxM_q8_K_generic_impl(int n, float* GGML_RESTRICT s,
                                                 size_t bs,
  ...
  for (int x = 0; x < nc / ncols_interleaved; x++) {
    ...
    for (int l = 0; l < nb; l++) {
      for (int k = 0; k < (qk / (2 * blocklen)); k++) {
        const int base_l =
            (k / blocks_per_half) * 128 + (k % blocks_per_half) * blocklen;
        ...
        for (int j = 0; j < ncols_interleaved; j++) {
          const int8_t scale_l =
              b_ptr[l].scales[scale_idx_l * ncols_interleaved + j];
          const int8_t scale_h =
              b_ptr[l].scales[scale_idx_h * ncols_interleaved + j];
          ...
          for (int i = 0; i < blocklen; i++) {
            const int ql_pos =
                k * ncols_interleaved * blocklen + j * blocklen + i;
            ...
            const int qh_offset_l =
                qh_chunk_l * (blocklen * ncols_interleaved) + j * blocklen +
                qh_pos_l;
            ...
            const int qh_offset_h =
                qh_chunk_h * (blocklen * ncols_interleaved) + j * blocklen +
                qh_pos_h;
          ...
          sumf[j] += (sumi_l * scale_l + sumi_h * scale_h) *
                     GGML_CPU_FP16_TO_FP32(b_ptr[l].d[j]) * a_ptr[l].d;
        }
      }
    }
...
template <int M, int N>
static void ggml_gemm_q6_K_NxM_q8_K_generic_impl(int n, float* GGML_RESTRICT s,
                                                 size_t bs,
                                                 const void* GGML_RESTRICT vx,
                                                 const void* GGML_RESTRICT vy,
                                                 int nr, int nc) {
    ...
    for (int x = 0; x < nc / ncols_interleaved; x++) {
      ...
      for (int l = 0; l < nb; l++) {
        for (int k = 0; k < (qk / (2 * blocklen)); k++) {
          ...
          const int q8_base = (k / blocks_per_half) * q8_half_stride +
                              (k % blocks_per_half) * (blocklen * 4);
            ...
            for (int j = 0; j < ncols_interleaved; j++) {
              ...
              for (int i = 0; i < blocklen; i++) {
                ...
                const int qh_offset_l =
                    ...
                    qh_pos_l;
                const int hi_2_l =
                    (b_ptr[l].qh[qh_offset_l] >> qh_shift_l) & 0x3;
                ...
                const int qh_offset_h =
                    ...
                    qh_pos_h;
                const int hi_2_h =
                    (b_ptr[l].qh[qh_offset_h] >> qh_shift_h) & 0x3;
                ...
                const int8_t q8_h =
                    a_ptr[l].qs[q8_base + m * blocklen + i + q8_low_high_step];

                sumi_l += q_l * q8_l;
                sumi_h += q_h * q8_h;
              }

              sumf[m][j] += (sumi_l * scale_l + sumi_h * scale_h) *
                            GGML_CPU_FP16_TO_FP32(b_ptr[l].d[j]) *
                            a_ptr[l].d[m];
            }
          }
        }
...
template <int M, int N>
static void ggml_gemv_q5_K_NxM_q8_K_generic_impl(int n, float* GGML_RESTRICT s,
                                                 size_t bs,
                                                 const void* GGML_RESTRICT vx,
                                                 const void* GGML_RESTRICT vy,
                                                 int nr, int nc) {
  ...
  for (int x = 0; x < nc / ncols_interleaved; x++) {
    ...
    for (int l = 0; l < nb; l++) {
      for (int sb = 0; sb < 8; sb++) {
        memcpy(utmp + sb * 4, b_ptr[l].scales + sb * K_SCALE_SIZE,
               K_SCALE_SIZE);
      ...
      for (int k = 0; k < (qk / (2 * blocklen)); k++) {
        ...
        uint8_t* scales_0 =
            (uint8_t*)utmp + (k / (32 / blocklen)) * scale_stride;
        uint8_t* scales_1 =
            (uint8_t*)utmp + (k / (32 / blocklen)) * scale_stride + 16;
        ...
        for (int j = 0; j < ncols_interleaved; j++) {
          ...
          for (int i = 0; i < blocklen; ++i) {
            ...
            const int v0 =
                (int8_t)((b_ptr[l].qs[b_qs_offset] & 0xF) | (h0 << 4));
            const int v1 =
                (int8_t)((b_ptr[l].qs[b_qs_offset] >> 4) | (h1 << 4));

            const int q8_offset = (k / (32 / blocklen)) * 64 +
                                  (k % (32 / blocklen)) * blocklen + i;
...
template <int M, int N>
static void ggml_gemm_q5_K_NxM_q8_K_generic_impl(int n, float* GGML_RESTRICT s,
                                                 size_t bs,
                                                 const void* GGML_RESTRICT vx,
                                                 const void* GGML_RESTRICT vy,
                                                 int nr, int nc) {
            ...
            for (int j = 0; j < ncols_interleaved; j++) {
              ...
              for (int i = 0; i < blocklen; ++i) {
                ...
                const int b_qh_offset =
                    qh_chunk * (blocklen * ncols_interleaved) + j * blocklen +
                    qh_pos;
      ...
      for (int k = 0; k < (qk / (2 * blocklen)); k++) {
        ...
        for (int j = 0; j < ncols_interleaved; j++) {
          ...
          for (int i = 0; i < blocklen; ++i) {
            ...
            sumi2 =
                (v1 * a_ptr[l].qs[(k / 8) * 64 + (k % 8) * blocklen + i + 32]);
            sumi1 = sumi1 * scales_0[j];
            sumi2 = sumi2 * scales_1[j];
            sumi += sumi1 + sumi2;
          }
      ...
      for (int k = 0; k < (qk / (4 * blocklen)); k++) {
        ...
        for (int j = 0; j < ncols_interleaved; j++) {
          ...
          for (int i = 0; i < blocklen; ++i) {
            ...
            sumi2 = (v1 *
                     a_ptr[l].qs[(k >> 2) * 128 + (k % 4) * blocklen + i + 32]);
            sumi3 = (v2 *
                     a_ptr[l].qs[(k >> 2) * 128 + (k % 4) * blocklen + i + 64]);
            sumi4 = (v3 *
                     a_ptr[l].qs[(k >> 2) * 128 + (k % 4) * blocklen + i + 96]);
...
void ggml_gemv_q5_K_8x4_q8_K_generic(int n, float* GGML_RESTRICT s, size_t bs,
                                     const void* GGML_RESTRICT vx,
                                     const void* GGML_RESTRICT vy, int nr,
                                     int nc) {
...
void ggml_gemv_q5_K_8x8_q8_K_generic(int n, float* GGML_RESTRICT s, size_t bs,
                                     const void* GGML_RESTRICT vx,
                                     const void* GGML_RESTRICT vy, int nr,
                                     int nc) {
...
void ggml_gemv_q6_K_8x8_q8_K_generic(int n, float* GGML_RESTRICT s, size_t bs,
                                     const void* GGML_RESTRICT vx,
                                     const void* GGML_RESTRICT vy, int nr,
                                     int nc) {
    ...
    for (int l = 0; l < nb; l++) {
      for (int k = 0; k < (qk / (2 * blocklen)); k++) {
        for (int j = 0; j < ncols_interleaved; j++) {
          ...
          for (int i = 0; i < blocklen; ++i) {
            const int v0 =
                kvalues_iq4nl[b_ptr[l].qs[k * ncols_interleaved * blocklen +
                                          j * blocklen + i] &
                              0x0F];
            const int v1 =
                kvalues_iq4nl[b_ptr[l].qs[k * ncols_interleaved * blocklen +
...
void ggml_gemv_mxfp4_4x4_q8_0_generic(int n, float* GGML_RESTRICT s, size_t bs,
                                      const void* GGML_RESTRICT vx,
                                      const void* GGML_RESTRICT vy, int nr,
                                      int nc) {
  const int qk = QK8_0;
    ...
    for (int l = 0; l < nb; l++) {
      for (int k = 0; k < (qk / (2 * blocklen)); k++) {
        for (int j = 0; j < ncols_interleaved; j++) {
          ...
          for (int i = 0; i < blocklen; ++i) {
            const int v0 =
                kvalues_mxfp4[b_ptr[l].qs[k * ncols_interleaved * blocklen +
                                          j * blocklen + i] &
                              0x0F];
            const int v1 =
                kvalues_mxfp4[b_ptr[l].qs[k * ncols_interleaved * blocklen +
          ...
          sumf[j] += sumi * GGML_CPU_E8M0_TO_FP32_HALF(b_ptr[l].e[j]) *
                     GGML_CPU_FP16_TO_FP32(a_ptr[l].d);
        }
      }
    }
...
void ggml_gemv_mxfp4_8x8_q8_0_generic(int n, float* GGML_RESTRICT s, size_t bs,
                                      const void* GGML_RESTRICT vx,
                                      const void* GGML_RESTRICT vy, int nr,
                                      int nc) {
  const int qk = QK8_0;
...
void ggml_gemv_q8_0_4x4_q8_0_generic(int n, float* GGML_RESTRICT s, size_t bs,
                                     const void* GGML_RESTRICT vx,
                                     const void* GGML_RESTRICT vy, int nr,
                                     int nc) {
  const int qk = QK8_0;
...
void ggml_gemv_q8_0_4x8_q8_0_generic(int n, float* GGML_RESTRICT s, size_t bs,
                                     const void* GGML_RESTRICT vx,
                                     const void* GGML_RESTRICT vy, int nr,
                                     int nc) {
  const int qk = QK8_0;
...
#if defined __riscv_zvfh
void ggml_gemv_q4_0_16x1_q8_0_generic(int n, float* GGML_RESTRICT s, size_t bs,
                                      const void* GGML_RESTRICT vx,
                                      const void* GGML_RESTRICT vy, int nr,
                                      int nc) {
  const int qk = QK8_0;
...
void ggml_gemv_q4_K_16x1_q8_K_generic(int n, float* GGML_RESTRICT s, size_t bs,
                                      const void* GGML_RESTRICT vx,
                                      const void* GGML_RESTRICT vy, int nr,
                                      int nc) {
  const int qk = QK_K;
...
void ggml_gemv_iq4_nl_16x1_q8_0_generic(int n, float* GGML_RESTRICT s,
...
void ggml_gemv_q8_0_16x1_q8_0_generic(int n, float* GGML_RESTRICT s, size_t bs,
                                      const void* GGML_RESTRICT vx,
                                      const void* GGML_RESTRICT vy, int nr,
                                      int nc) {
  const int qk = QK8_0;
...
void ggml_gemv_q2_K_16x1_q8_K_generic(int n, float* GGML_RESTRICT s, size_t bs,
                                      const void* GGML_RESTRICT vx,
                                      const void* GGML_RESTRICT vy, int nr,
                                      int nc) {
  assert(n % QK_K == 0);
  ...
  // Layout: Even-Low(0,2,4,6), Odd-Low(1,3,5,7), Even-High(8...),
  // Odd-High(9...)
  ...
  for (int col_tile = 0; col_tile < nc; col_tile += 16) {
    ...
    for (int k_block = 0; k_block < nb; ++k_block) {
      ...
      for (int sb = 0; sb < 16; ++sb) {
        ...
        int byte_base;
        if (sb < 8)
          byte_base = (sb % 2 == 0) ? 0 : 16;
        else
          byte_base = (sb % 2 == 0) ? 32 : 48;
              ...
              for (int i = 0; i < blocklen; ++i) {
                ...
                const int v1 =
                    (int8_t)(b_ptr[l].qs[k * ncols_interleaved * blocklen +
                             ...
                             4);
                sumi1 =
                    (v0 * a_ptr[l].qs[(k / 8) * 256 + (k % 8) * 4 * blocklen +
                                      m * blocklen + i]);
                sumi2 =
                    (v1 * a_ptr[l].qs[(k / 8) * 256 + (k % 8) * 4 * blocklen +
                                      m * blocklen + i + 128]);
                sumi1 = sumi1 * scales_0[j];
                sumi2 = sumi2 * scales_1[j];
                sumi += sumi1 + sumi2;
        ...
        for (int k = 0; k < (qk / (4 * blocklen)); k++) {
          ...
          for (int m = 0; m < 4; m++) {
            for (int j = 0; j < ncols_interleaved; j++) {
              ...
              for (int i = 0; i < blocklen; ++i) {
                ...
                const int v3 =
                    (int8_t)((b_ptr[l].qs[k * ncols_interleaved * blocklen +
                             ...
                             3);
                sumi1 =
                    (v0 * a_ptr[l].qs[(k >> 2) * 512 + (k % 4) * 4 * blocklen +
                                      m * blocklen + i]);
                sumi2 =
                    (v1 * a_ptr[l].qs[(k >> 2) * 512 + (k % 4) * 4 * blocklen +
                                      m * blocklen + i + 128]);
                sumi3 =
                    (v2 * a_ptr[l].qs[(k >> 2) * 512 + (k % 4) * 4 * blocklen +
                                      m * blocklen + i + 256]);
                sumi4 =
                    (v3 * a_ptr[l].qs[(k >> 2) * 512 + (k % 4) * 4 * blocklen +
                                      m * blocklen + i + 384]);
                sumi1 = sumi1 * (scales_0[offset] & 0xF);
                sumi2 = sumi2 * (scales_1[offset] & 0xF);
                sumi3 = sumi3 * (scales_2[offset] & 0xF);
              ...
              sumf[m][j] +=
                  sumi * GGML_FP16_TO_FP32(b_ptr[l].d[j]) * a_ptr[l].d[m];
            }
          }
        }
        for (int sb = 0; sb < 8; sb++) {
          ...
          for (int m = 0; m < 4; m++) {
            ...
            for (int j = 0; j < ncols_interleaved; j++) {
              ...
              sum_minf[m][j] +=
                  (mins_prod)*GGML_FP16_TO_FP32(b_ptr[l].dmin[j]) *
                  a_ptr[l].d[m];
            }
          }
        }
...
void ggml_gemm_q5_K_8x4_q8_K_generic(int n, float* GGML_RESTRICT s, size_t bs,
                                     const void* GGML_RESTRICT vx,
                                     const void* GGML_RESTRICT vy, int nr,
                                     int nc) {
...
void ggml_gemm_q5_K_8x8_q8_K_generic(int n, float* GGML_RESTRICT s, size_t bs,
                                     const void* GGML_RESTRICT vx,
                                     const void* GGML_RESTRICT vy, int nr,
                                     int nc) {
...
void ggml_gemm_q6_K_8x8_q8_K_generic(int n, float* GGML_RESTRICT s, size_t bs,
                                     const void* GGML_RESTRICT vx,
                                     const void* GGML_RESTRICT vy, int nr,
                                     int nc) {
          ...
          for (int k = 0; k < (qk / (2 * blocklen)); k++) {
            for (int m = 0; m < 4; m++) {
              for (int j = 0; j < ncols_interleaved; j++) {
                ...
                for (int i = 0; i < blocklen; ++i) {
                  const int v0 =
                      kvalues_iq4nl[b_ptr[l]
                                        .qs[k * ncols_interleaved * blocklen +
                                            j * blocklen + i] &
                                    0x0F];
                  const int v1 =
                      kvalues_iq4nl[b_ptr[l]
...
void ggml_gemm_mxfp4_4x4_q8_0_generic(int n, float* GGML_RESTRICT s, size_t bs,
                                      const void* GGML_RESTRICT vx,
                                      const void* GGML_RESTRICT vy, int nr,
                                      int nc) {
  const int qk = QK8_0;
        ...
        for (int k = 0; k < (qk / (2 * blocklen)); k++) {
          for (int m = 0; m < 4; m++) {
            for (int j = 0; j < ncols_interleaved; j++) {
              ...
              sumf[m][j] += sumi * GGML_CPU_E8M0_TO_FP32_HALF(b_ptr[l].e[j]) *
                            GGML_CPU_FP16_TO_FP32(a_ptr[l].d[m]);
            }
          }
        }
...
void ggml_gemm_mxfp4_8x8_q8_0_generic(int n, float* GGML_RESTRICT s, size_t bs,
                                      const void* GGML_RESTRICT vx,
                                      const void* GGML_RESTRICT vy, int nr,
                                      int nc) {
  const int qk = QK8_0;
...
void ggml_gemm_q8_0_4x4_q8_0_generic(int n, float* GGML_RESTRICT s, size_t bs,
                                     const void* GGML_RESTRICT vx,
                                     const void* GGML_RESTRICT vy, int nr,
                                     int nc) {
  const int qk = QK8_0;
        ...
        for (int k = 0; k < (qk / blocklen); k++) {
          for (int m = 0; m < 4; m++) {
            for (int j = 0; j < ncols_interleaved; j++) {
              sumi = 0;
              for (int i = 0; i < blocklen; ++i) {
                const int v0 = b_ptr[l].qs[k * ncols_interleaved * blocklen +
                                           j * blocklen + i];
...
void ggml_gemm_q8_0_4x8_q8_0_generic(int n, float* GGML_RESTRICT s, size_t bs,
                                     const void* GGML_RESTRICT vx,
                                     const void* GGML_RESTRICT vy, int nr,
                                     int nc) {
  const int qk = QK8_0;
...
// Only enable these for RISC-V.
#if defined __riscv_zvfh
void ggml_gemm_q4_0_16x1_q8_0_generic(int n, float* GGML_RESTRICT s, size_t bs,
                                      const void* GGML_RESTRICT vx,
                                      const void* GGML_RESTRICT vy, int nr,
                                      int nc) {
  const int qk = QK8_0;
...
void ggml_gemm_q4_K_16x1_q8_K_generic(int n, float* GGML_RESTRICT s, size_t bs,
                                      const void* GGML_RESTRICT vx,
                                      const void* GGML_RESTRICT vy, int nr,
                                      int nc) {
  const int qk = QK_K;
    ...
    for (int x = 0; x < nc / ncols_interleaved; x++) {
      ...
      for (int l = 0; l < nb; l++) {
        ...
        for (int sb = 0; sb < 8; sb++) {
          ...
          for (int m = 0; m < 4; m++) {
            const int16_t bsums =
                a_ptr[l].bsums[sb * 8 + m] + a_ptr[l].bsums[sb * 8 + m + 4];
            for (int j = 0; j < ncols_interleaved; j++) {
              sum_minf[m][j] += min[j] * bsums *
                                GGML_CPU_FP16_TO_FP32(b_ptr[l].dmin[j]) *
                                a_ptr[l].d[m];
            }
          }
          ...
          for (int i = 0; i < QK4_0; i++) {
            for (int m = 0; m < 4; m++) {
              for (int j = 0; j < ncols_interleaved; j++) {
                ...
                const int v0 =
                    (int8_t)(b_ptr[l].qs[sb * 256 + i * 16 + j] & 0xF);
                const int v1 =
                    (int8_t)(b_ptr[l].qs[sb * 256 + i * 16 + j] >> 4);
...
void ggml_gemm_iq4_nl_16x1_q8_0_generic(int n, float* GGML_RESTRICT s,
                                        size_t bs, const void* GGML_RESTRICT vx,
                                        const void* GGML_RESTRICT vy, int nr,
                                        int nc) {
  const int qk = QK8_0;
...
void ggml_gemm_q8_0_16x1_q8_0_generic(int n, float* GGML_RESTRICT s, size_t bs,
                                      const void* GGML_RESTRICT vx,
                                      const void* GGML_RESTRICT vy, int nr,
                                      int nc) {
  const int qk = QK8_0;
...
void ggml_gemm_q2_K_16x1_q8_K_generic(int n, float* GGML_RESTRICT s, size_t bs,
                                      const void* GGML_RESTRICT vx,
                                      const void* GGML_RESTRICT vy, int nr,
                                      int nc) {
  assert(n % QK_K == 0);
  ...
  const int sb_perm[16] = {0, 4,  1, 5,  2,  6,  3,  7,
...
static block_q8_0x4 make_block_q8_0x4(block_q8_0* in,
...
static block_q4_0x16 make_block_q4_0x16(block_q4_0* in,
                                        unsigned int blck_size_interleave) {
  block_q4_0x16 out;
...
static block_q4_Kx16 make_block_q4_Kx16(block_q4_K* in,
                                        unsigned int blck_size_interleave) {
  block_q4_Kx16 out;
  // Delta(scale) and dmin values of the 16 Q4_K structures are copied onto the
  // output interleaved structure
  for (int i = 0; i < 16; i++) {
    out.d[i] = in[i].GGML_COMMON_AGGR_U.GGML_COMMON_AGGR_S.d;
  }
  ...
  if (blck_size_interleave == 1) {
    ...
    for (int i = 0; i < 4; i++) {
      for (int j = 0; j < 16; j++) {
        s[64 + i * 16 + j] =
            ((in[j].scales[i] & 192) >> 2) | (in[j].scales[i + 8] & 15);
        m[64 + i * 16 + j] = ((in[j].scales[i + 4] & 192) >> 2) |
                             ((in[j].scales[i + 8] & 240) >> 4);
      }
    }
    ...
    for (int i = 0; i < 64; i++) {
      out.scales[128 + i] = ((s[i] & 48) >> 4) | ((m[i] & 48) >> 2) |
                            (s[64 + i] & 48) | ((m[64 + i] & 48) << 2);
    }
  } else {
    GGML_ASSERT(false);
  }
...
static block_q2_Kx8 make_block_q2_Kx8(block_q2_K* in,
                                      unsigned int blck_size_interleave) {
  block_q2_Kx8 out;

  // Delta(scale) and dmin values of the eight Q2_K structures are copied onto
  // the output interleaved structure
  for (int i = 0; i < 8; i++) {
    out.d[i] = in[i].GGML_COMMON_AGGR_U.GGML_COMMON_AGGR_S.d;
  }
  ...
  // The below logic is designed so as to unpack and rearrange scales and mins
  // values in Q2_K Currently the Q2_K structure has 16 scales and 16 mins
  // packed in 16 bytes ( 4 bits for each value) The output Q2_Kx8 structure has
  // 128 bytes for storing scales and mins Every 16 byte is packed such that it
  // contains scales and mins for corresponding sub blocks from Q2_K structure
  // For eg - First 16 bytes contains 16 scales and 16 mins - each of first and
  // second sub blocks from different Q2_K structures
...
static block_q5_Kx8 make_block_q5_Kx8(block_q5_K* in,
                                      unsigned int blck_size_interleave) {
  block_q5_Kx8 out;
  // Delta(scale) and dmin values of the eight Q5_K structures are copied onto
  // the output interleaved structure
  for (int i = 0; i < 8; i++) {
    out.d[i] = in[i].GGML_COMMON_AGGR_U.GGML_COMMON_AGGR_S.d;
  }
  ...
  for (int i = 0; i < end / 4; ++i) {
    ...
    memcpy(&out.qh[dst_offset], &in[src_id].qh[src_offset],
           blck_size_interleave);
  }

  // The below logic is copied over from Q4_K
  // The point is to unpack all the scales and mins for each sub block every
  // time we load 12 bytes. Currently the Q5_K structure has 8 scales and 8 mins
  // packed in 12 bytes ( 6 bits for each value) The output Q5_Kx8 structure has
  // 96 bytes Every 12 byte is packed such that it contains scales and mins for
  // corresponding sub blocks from Q5_K structure For eg - First 12 bytes
  // contains 8 scales and 8 mins - each of first sub block from different Q5_K
  // structures
  uint8_t s[8], m[8];
...
static block_q6_Kx8 make_block_q6_Kx8(block_q6_K* in,
                                      unsigned int blck_size_interleave) {
  block_q6_Kx8 out;
  ...
  // The output Q6_Kx8 structure interleaves the 8 bit scales in the same
  // fashion as the quants Q6_K structure has an 8-bit scale per 16 elements ->
  // 16 scales scales: [0 bl0 0 bl1 ... 0 bl7][1 bl0 ... 1 bl7] ... [15 bl0 ...
  // 15 bl7]  (bl = block)
...
static block_q2_Kx16 make_block_q2_Kx16(const block_q2_K* in,
  ...
  for (int i = 0; i < end; ++i) {
    ...
    int dst_offset = i * blck_size_interleave;
    memcpy(&out.qs[dst_offset], &in[src_col_id].qs[src_offset],
           blck_size_interleave);
  }
...
static int repack_q4_K_to_q4_K_16_bl(struct ggml_tensor* t,
...
static int repack_q2_K_to_q2_K_16_bl(struct ggml_tensor* t,
                                     int interleave_block,
                                     const void* GGML_RESTRICT data,
                                     size_t data_size) {
  GGML_ASSERT(t->type == GGML_TYPE_Q2_K);
...
static int repack_q5_K_to_q5_K_8_bl(struct ggml_tensor* t, int interleave_block,
                                    const void* GGML_RESTRICT data,
                                    size_t data_size) {
...
static int repack_q6_K_to_q6_K_8_bl(struct ggml_tensor* t, int interleave_block,
                                    const void* GGML_RESTRICT data,
                                    size_t data_size) {
...
static block_iq4_nlx4 make_block_iq4_nlx4(block_iq4_nl* in,
                                          unsigned int blck_size_interleave) {
  block_iq4_nlx4 out;
...
static int repack_iq4_nl_to_iq4_nl_4_bl(struct ggml_tensor* t,
                                        int interleave_block,
                                        const void* GGML_RESTRICT data,
                                        size_t data_size) {
...
static block_iq4_nlx8 make_block_iq4_nlx8(block_iq4_nl* in,
                                          unsigned int blck_size_interleave) {
  block_iq4_nlx8 out;
...
static int repack_iq4_nl_to_iq4_nl_8_bl(struct ggml_tensor* t,
                                        int interleave_block,
                                        const void* GGML_RESTRICT data,
                                        size_t data_size) {
  GGML_ASSERT(t->type == GGML_TYPE_IQ4_NL);
...
static block_iq4_nlx16 make_block_iq4_nlx16(block_iq4_nl* in,
                                            unsigned int blck_size_interleave) {
  block_iq4_nlx16 out;
...
static int repack_iq4_nl_to_iq4_nl_16_bl(struct ggml_tensor* t,
                                         int interleave_block,
                                         const void* GGML_RESTRICT data,
                                         size_t data_size) {
  GGML_ASSERT(t->type == GGML_TYPE_IQ4_NL);
...
static block_mxfp4x4 make_block_mxfp4x4(block_mxfp4* in,
                                        unsigned int blck_size_interleave) {
  block_mxfp4x4 out;
...
static int repack_mxfp4_to_mxfp4_4_bl(struct ggml_tensor* t,
                                      int interleave_block,
                                      const void* GGML_RESTRICT data,
                                      size_t data_size) {
...
static block_mxfp4x8 make_block_mxfp4x8(block_mxfp4* in,
                                        unsigned int blck_size_interleave) {
  block_mxfp4x8 out;
...
static int repack_mxfp4_to_mxfp4_8_bl(struct ggml_tensor* t,
                                      int interleave_block,
                                      const void* GGML_RESTRICT data,
                                      size_t data_size) {
  GGML_ASSERT(t->type == GGML_TYPE_MXFP4);
...
// TODO: generalise.
template <>
int repack<block_q4_0, 4, 4>(struct ggml_tensor* t, const void* data,
                             size_t data_size) {
  return repack_q4_0_to_q4_0_4_bl(t, 4, data, data_size);
}

template <>
int repack<block_q4_0, 8, 4>(struct ggml_tensor* t, const void* data,
                             size_t data_size) {
  return repack_q4_0_to_q4_0_4_bl(t, 8, data, data_size);
}

template <>
int repack<block_q4_0, 8, 8>(struct ggml_tensor* t, const void* data,
                             size_t data_size) {
  return repack_q4_0_to_q4_0_8_bl(t, 8, data, data_size);
}

template <>
int repack<block_q4_K, 8, 8>(struct ggml_tensor* t, const void* data,
                             size_t data_size) {
  return repack_q4_K_to_q4_K_8_bl(t, 8, data, data_size);
}

template <>
int repack<block_q4_K, 4, 8>(struct ggml_tensor* t, const void* data,
                             size_t data_size) {
  return repack_q4_K_to_q4_K_8_bl(t, 4, data, data_size);
}

template <>
int repack<block_q2_K, 8, 8>(struct ggml_tensor* t, const void* data,
                             size_t data_size) {
  return repack_q2_K_to_q2_K_8_bl(t, 8, data, data_size);
}

template <>
int repack<block_q5_K, 4, 8>(struct ggml_tensor* t, const void* data,
                             size_t data_size) {
  return repack_q5_K_to_q5_K_8_bl(t, 4, data, data_size);
}

template <>
int repack<block_q5_K, 8, 8>(struct ggml_tensor* t, const void* data,
                             size_t data_size) {
  return repack_q5_K_to_q5_K_8_bl(t, 8, data, data_size);
}

template <>
int repack<block_q6_K, 4, 8>(struct ggml_tensor* t, const void* data,
                             size_t data_size) {
  return repack_q6_K_to_q6_K_8_bl(t, 4, data, data_size);
}

template <>
int repack<block_q6_K, 8, 8>(struct ggml_tensor* t, const void* data,
                             size_t data_size) {
  return repack_q6_K_to_q6_K_8_bl(t, 8, data, data_size);
}

template <>
int repack<block_iq4_nl, 4, 4>(struct ggml_tensor* t, const void* data,
                               size_t data_size) {
  return repack_iq4_nl_to_iq4_nl_4_bl(t, 4, data, data_size);
}

// TODO: needs to be revisited
// template <> int repack<block_iq4_nl, 8, 4>(struct ggml_tensor * t, const void
// * data, size_t data_size) {
//    return repack_iq4_nl_to_iq4_nl_4_bl(t, 8, data, data_size);
//}

template <>
int repack<block_iq4_nl, 8, 8>(struct ggml_tensor* t, const void* data,
                               size_t data_size) {
  return repack_iq4_nl_to_iq4_nl_8_bl(t, 8, data, data_size);
}

template <>
int repack<block_mxfp4, 4, 4>(struct ggml_tensor* t, const void* data,
                              size_t data_size) {
  return repack_mxfp4_to_mxfp4_4_bl(t, 4, data, data_size);
}

template <>
int repack<block_mxfp4, 8, 8>(struct ggml_tensor* t, const void* data,
                              size_t data_size) {
  return repack_mxfp4_to_mxfp4_8_bl(t, 8, data, data_size);
}

template <>
int repack<block_q8_0, 4, 4>(struct ggml_tensor* t, const void* data,
                             size_t data_size) {
  return repack_q8_0_to_q8_0_4_bl(t, 4, data, data_size);
}

template <>
int repack<block_q8_0, 8, 4>(struct ggml_tensor* t, const void* data,
                             size_t data_size) {
...
#if defined __riscv_zvfh
template <>
int repack<block_q4_0, 1, 16>(struct ggml_tensor* t, const void* data,
                              size_t data_size) {
  return repack_q4_0_to_q4_0_16_bl(t, 1, data, data_size);
}

template <>
int repack<block_q4_K, 1, 16>(struct ggml_tensor* t, const void* data,
                              size_t data_size) {
  return repack_q4_K_to_q4_K_16_bl(t, 1, data, data_size);
}

template <>
int repack<block_iq4_nl, 1, 16>(struct ggml_tensor* t, const void* data,
                                size_t data_size) {
  return repack_iq4_nl_to_iq4_nl_16_bl(t, 1, data, data_size);
}

template <>
int repack<block_q8_0, 1, 16>(struct ggml_tensor* t, const void* data,
                              size_t data_size) {
  return repack_q8_0_to_q8_0_16_bl(t, 1, data, data_size);
}

template <>
int repack<block_q2_K, 1, 16>(struct ggml_tensor* t, const void* data,
                              size_t data_size) {
...
template <>
void gemv<block_q4_0, 4, 4, GGML_TYPE_Q8_0>(int n, float* s, size_t bs,
...
template <>
void gemv<block_q4_0, 8, 4, GGML_TYPE_Q8_0>(int n, float* s, size_t bs,
                                            const void* vx, const void* vy,
                                            int nr, int nc) {
...
template <>
void gemv<block_q2_K, 8, 8, GGML_TYPE_Q8_K>(int n, float* s, size_t bs,
                                            const void* vx, const void* vy,
                                            int nr, int nc) {
...
template <>
void gemv<block_q4_K, 4, 8, GGML_TYPE_Q8_K>(int n, float* s, size_t bs,
                                            const void* vx, const void* vy,
                                            int nr, int nc) {
...
template <>
void gemv<block_q5_K, 4, 8, GGML_TYPE_Q8_K>(int n, float* s, size_t bs,
                                            const void* vx, const void* vy,
                                            int nr, int nc) {
...
template <>
void gemv<block_q5_K, 8, 8, GGML_TYPE_Q8_K>(int n, float* s, size_t bs,
                                            const void* vx, const void* vy,
                                            int nr, int nc) {
...
template <>
void gemv<block_q6_K, 4, 8, GGML_TYPE_Q8_K>(int n, float* s, size_t bs,
                                            const void* vx, const void* vy,
                                            int nr, int nc) {
...
template <>
void gemv<block_q6_K, 8, 8, GGML_TYPE_Q8_K>(int n, float* s, size_t bs,
                                            const void* vx, const void* vy,
                                            int nr, int nc) {
...
template <>
void gemv<block_iq4_nl, 4, 4, GGML_TYPE_Q8_0>(int n, float* s, size_t bs,
                                              const void* vx, const void* vy,
                                              int nr, int nc) {
...
template <>
void gemv<block_iq4_nl, 8, 8, GGML_TYPE_Q8_0>(int n, float* s, size_t bs,
                                              const void* vx, const void* vy,
                                              int nr, int nc) {
...
template <>
void gemv<block_mxfp4, 4, 4, GGML_TYPE_Q8_0>(int n, float* s, size_t bs,
                                             const void* vx, const void* vy,
                                             int nr, int nc) {
...
template <>
void gemv<block_mxfp4, 8, 8, GGML_TYPE_Q8_0>(int n, float* s, size_t bs,
                                             const void* vx, const void* vy,
                                             int nr, int nc) {
...
template <>
void gemv<block_q8_0, 4, 4, GGML_TYPE_Q8_0>(int n, float* s, size_t bs,
                                            const void* vx, const void* vy,
                                            int nr, int nc) {
...
template <>
void gemv<block_q8_0, 8, 4, GGML_TYPE_Q8_0>(int n, float* s, size_t bs,
                                            const void* vx, const void* vy,
                                            int nr, int nc) {
...
#if defined __riscv_zvfh
template <>
void gemv<block_q4_0, 1, 16, GGML_TYPE_Q8_0>(int n, float* s, size_t bs,
                                             const void* vx, const void* vy,
                                             int nr, int nc) {
...
template <>
void gemv<block_q4_K, 1, 16, GGML_TYPE_Q8_K>(int n, float* s, size_t bs,
                                             const void* vx, const void* vy,
                                             int nr, int nc) {
...
template <>
void gemv<block_iq4_nl, 1, 16, GGML_TYPE_Q8_0>(int n, float* s, size_t bs,
                                               const void* vx, const void* vy,
                                               int nr, int nc) {
...
template <>
void gemv<block_q8_0, 1, 16, GGML_TYPE_Q8_0>(int n, float* s, size_t bs,
                                             const void* vx, const void* vy,
                                             int nr, int nc) {
...
template <>
void gemv<block_q2_K, 1, 16, GGML_TYPE_Q8_K>(int n, float* s, size_t bs,
                                             const void* vx, const void* vy,
                                             int nr, int nc) {
...
template <>
void gemm<block_q4_0, 4, 4, GGML_TYPE_Q8_0>(int n, float* s, size_t bs,
                                            const void* vx, const void* vy,
                                            int nr, int nc) {
...
template <>
void gemm<block_q4_0, 8, 4, GGML_TYPE_Q8_0>(int n, float* s, size_t bs,
                                            const void* vx, const void* vy,
                                            int nr, int nc) {
...
template <>
void gemm<block_q4_0, 8, 8, GGML_TYPE_Q8_0>(int n, float* s, size_t bs,
                                            const void* vx, const void* vy,
                                            int nr, int nc) {
...
template <>
void gemm<block_q2_K, 8, 8, GGML_TYPE_Q8_K>(int n, float* s, size_t bs,
                                            const void* vx, const void* vy,
                                            int nr, int nc) {
...
template <>
void gemm<block_q4_K, 4, 8, GGML_TYPE_Q8_K>(int n, float* s, size_t bs,
                                            const void* vx, const void* vy,
                                            int nr, int nc) {
...
template <>
void gemm<block_q5_K, 4, 8, GGML_TYPE_Q8_K>(int n, float* s, size_t bs,
                                            const void* vx, const void* vy,
                                            int nr, int nc) {
...
template <>
void gemm<block_q5_K, 8, 8, GGML_TYPE_Q8_K>(int n, float* s, size_t bs,
                                            const void* vx, const void* vy,
                                            int nr, int nc) {
...
template <>
void gemm<block_q6_K, 4, 8, GGML_TYPE_Q8_K>(int n, float* s, size_t bs,
                                            const void* vx, const void* vy,
                                            int nr, int nc) {
...
template <>
void gemm<block_q6_K, 8, 8, GGML_TYPE_Q8_K>(int n, float* s, size_t bs,
                                            const void* vx, const void* vy,
                                            int nr, int nc) {
...
template <>
void gemm<block_iq4_nl, 4, 4, GGML_TYPE_Q8_0>(int n, float* s, size_t bs,
                                              const void* vx, const void* vy,
                                              int nr, int nc) {
...
template <>
void gemm<block_iq4_nl, 8, 8, GGML_TYPE_Q8_0>(int n, float* s, size_t bs,
                                              const void* vx, const void* vy,
                                              int nr, int nc) {
...
template <>
void gemm<block_mxfp4, 4, 4, GGML_TYPE_Q8_0>(int n, float* s, size_t bs,
                                             const void* vx, const void* vy,
                                             int nr, int nc) {
...
template <>
void gemm<block_mxfp4, 8, 8, GGML_TYPE_Q8_0>(int n, float* s, size_t bs,
                                             const void* vx, const void* vy,
                                             int nr, int nc) {
...
template <>
void gemm<block_q8_0, 4, 4, GGML_TYPE_Q8_0>(int n, float* s, size_t bs,
                                            const void* vx, const void* vy,
                                            int nr, int nc) {
...
template <>
void gemm<block_q8_0, 8, 4, GGML_TYPE_Q8_0>(int n, float* s, size_t bs,
                                            const void* vx, const void* vy,
                                            int nr, int nc) {
...
#if defined __riscv_zvfh
template <>
void gemm<block_q4_0, 1, 16, GGML_TYPE_Q8_0>(int n, float* s, size_t bs,
                                             const void* vx, const void* vy,
                                             int nr, int nc) {
...
template <>
void gemm<block_q4_K, 1, 16, GGML_TYPE_Q8_K>(int n, float* s, size_t bs,
                                             const void* vx, const void* vy,
                                             int nr, int nc) {
...
template <>
void gemm<block_iq4_nl, 1, 16, GGML_TYPE_Q8_0>(int n, float* s, size_t bs,
                                               const void* vx, const void* vy,
                                               int nr, int nc) {
...
template <>
void gemm<block_q8_0, 1, 16, GGML_TYPE_Q8_0>(int n, float* s, size_t bs,
                                             const void* vx, const void* vy,
                                             int nr, int nc) {
...
template <>
void gemm<block_q2_K, 1, 16, GGML_TYPE_Q8_K>(int n, float* s, size_t bs,
                                             const void* vx, const void* vy,
                                             int nr, int nc) {
...
class tensor_traits_base : public ggml::cpu::tensor_traits {
 public:
  virtual int repack(struct ggml_tensor* t, const void* data,
                     size_t data_size) = 0;
};
...
class tensor_traits : public tensor_traits_base {
  bool work_size(int /* n_threads */, const struct ggml_tensor* op,
  ...
  void forward_mul_mat_one_chunk(ggml_compute_params* params, ggml_tensor* op,
    ...
    const char* src1_ptr =
        (const char*)params->wdata + (i11 + i12 * ne11) * src1_col_stride;
    ...
    for (int iter = nrows - (nrows % 4); iter < nrows; iter++) {
      gemv<BLOC_TYPE, INTER_SIZE, NB_COLS, PARAM_TYPE>(
          ne00, (float*)(dst_ptr + (iter * nb1)) + src0_start, ne01,
          src0_ptr + src0_start * nb01, src1_ptr + (src1_col_stride * iter),
          1 /* nrows */, ncols);
    }
  }
  ...
  void forward_mul_mat(ggml_compute_params* params, ggml_tensor* op) {
    ...
    // INFO: Quantization is done in planes to avoid extra complexity in
    // chunking. Flattening dimensions not multiple of INTER_SIZE would require
    // extra handling depending on how the planes are broadcast.
    for (int64_t i12 = 0; i12 < ne12; i12++) {
      ...
      for (int64_t i11 = ith * 4; i11 < ne11 - ne11 % 4; i11 += nth * 4) {
        ggml_quantize_mat_t<INTER_SIZE, PARAM_TYPE>(
            (float*)(data_ptr + i11 * nb11), (void*)(wdata_ptr + i11 * nbw1), 4,
            ne10);
      }
    ...
    // When we flatten we need to address dimensions not multiple of the q8
    // INTER_SIZE to route them thorugh GEMV. nchunk1 = ne12 also avoids messing
    // the chunking for models with no 3d tensors to avoid affecting their
    // performance
    int64_t nchunk1 = ne12;

    // Ensure minimum chunk size to avoid alignment issues with high thread
    // counts Minimum chunk size should be at least NB_COLS to prevent
    // overlapping chunks after alignment
    const int64_t min_chunk_size = NB_COLS;
    if (nchunk0 > 0 && (nr0 / nchunk0) < min_chunk_size &&
        nr0 >= min_chunk_size) {
    ...
    // Ensure nchunk doesn't exceed the number of rows divided by minimum chunk
    // size This prevents creating too many tiny chunks that could overlap after
    // alignment
    ...
    // The first chunk comes from our thread_id, the rest will get
    // auto-assigned.
    ...
    while (current_chunk < nchunk0 * nchunk1) {
      ...
      // The chunk size limiting above ensures chunks are large enough to
      // prevent overlaps
      ...
      forward_mul_mat_one_chunk(params, dst, src0_start, src0_end, src1_start,
                                src1_end);
  ...
  void forward_mul_mat_id(ggml_compute_params* params, ggml_tensor* op) {
    ...
    // total of [n_as][ne12 + 1] elements of type mmid_row_mapping (2*int32_t =
    // int64_t)
    auto* matrix_row_counts = (int64_t*)(wdata_src1_end);  // [n_as]
    struct mmid_row_mapping* matrix_rows =
        (struct mmid_row_mapping*)(matrix_row_counts + n_as);  // [n_as][ne12]
    ...
    for (int cur_a = 0; cur_a < n_as; ++cur_a) {
      ...
      src0_cur_start =
          (src0_cur_start % NB_COLS)
              ? src0_cur_start + NB_COLS - (src0_cur_start % NB_COLS)
              : src0_cur_start;
      src0_cur_end = (src0_cur_end % NB_COLS)
                         ? src0_cur_end + NB_COLS - (src0_cur_end % NB_COLS)
                         : src0_cur_end;
      ...
      for (int ir1 = 0; ir1 < nr1; ir1++) {
        ...
        gemv<BLOC_TYPE, INTER_SIZE, NB_COLS, PARAM_TYPE>(
            ...
            ne01, src0_cur + src0_cur_start * nb01, src1_col, 1,
...
#undef MMID_MATRIX_ROW
  ...
  int repack(struct ggml_tensor* t, const void* data,
             size_t data_size) override {
    GGML_LOG_DEBUG("%s: repack tensor %s with %s_%dx%d\n", __func__, t->name,
                   ggml_type_name(t->type), (int)NB_COLS, (int)INTER_SIZE);
    return ggml::cpu::repack::repack<BLOC_TYPE, INTER_SIZE, NB_COLS>(t, data,
                                                                     data_size);
  }
};
...
static const ggml::cpu::tensor_traits* ggml_repack_get_optimal_repack_type(
    const struct ggml_tensor* cur) {
  // instance for Q4
  static const ggml::cpu::repack::tensor_traits<block_q4_0, 4, 4,
                                                GGML_TYPE_Q8_0>
      q4_0_4x4_q8_0;
  static const ggml::cpu::repack::tensor_traits<block_q4_0, 8, 4,
                                                GGML_TYPE_Q8_0>
      q4_0_4x8_q8_0;
  static const ggml::cpu::repack::tensor_traits<block_q4_0, 8, 8,
                                                GGML_TYPE_Q8_0>
      q4_0_8x8_q8_0;

  // instance for Q4_K
  static const ggml::cpu::repack::tensor_traits<block_q4_K, 4, 8,
                                                GGML_TYPE_Q8_K>
      q4_K_8x4_q8_K;
  static const ggml::cpu::repack::tensor_traits<block_q4_K, 8, 8,
                                                GGML_TYPE_Q8_K>
      q4_K_8x8_q8_K;

  // instance for Q5_K
  static const ggml::cpu::repack::tensor_traits<block_q5_K, 4, 8,
                                                GGML_TYPE_Q8_K>
      q5_K_8x4_q8_K;
  static const ggml::cpu::repack::tensor_traits<block_q5_K, 8, 8,
                                                GGML_TYPE_Q8_K>
      q5_K_8x8_q8_K;

  // instance for Q6_K
  static const ggml::cpu::repack::tensor_traits<block_q6_K, 4, 8,
                                                GGML_TYPE_Q8_K>
      q6_K_8x4_q8_K;
  static const ggml::cpu::repack::tensor_traits<block_q6_K, 8, 8,
                                                GGML_TYPE_Q8_K>
      q6_K_8x8_q8_K;

  // instance for Q2
  static const ggml::cpu::repack::tensor_traits<block_q2_K, 8, 8,
                                                GGML_TYPE_Q8_K>
      q2_K_8x8_q8_K;

  // instance for IQ4
  static const ggml::cpu::repack::tensor_traits<block_iq4_nl, 4, 4,
                                                GGML_TYPE_Q8_0>
      iq4_nl_4x4_q8_0;
  static const ggml::cpu::repack::tensor_traits<block_iq4_nl, 8, 8,
                                                GGML_TYPE_Q8_0>
      iq4_nl_8x8_q8_0;

  // instance for MXFP4
  static const ggml::cpu::repack::tensor_traits<block_mxfp4, 4, 4,
                                                GGML_TYPE_Q8_0>
      mxfp4_4x4_q8_0;
  static const ggml::cpu::repack::tensor_traits<block_mxfp4, 8, 8,
                                                GGML_TYPE_Q8_0>
      mxfp4_8x8_q8_0;

  // instance for Q8_0
  static const ggml::cpu::repack::tensor_traits<block_q8_0, 4, 4,
                                                GGML_TYPE_Q8_0>
      q8_0_4x4_q8_0;
  static const ggml::cpu::repack::tensor_traits<block_q8_0, 8, 4,
                                                GGML_TYPE_Q8_0>
      q8_0_4x8_q8_0;
  ...
  // an interleave of 1.
#if defined __riscv_zvfh
  static const ggml::cpu::repack::tensor_traits<block_q4_0, 1, 16,
                                                GGML_TYPE_Q8_0>
      q4_0_16x1_q8_0;
  static const ggml::cpu::repack::tensor_traits<block_q4_K, 1, 16,
                                                GGML_TYPE_Q8_K>
      q4_K_16x1_q8_K;
  static const ggml::cpu::repack::tensor_traits<block_iq4_nl, 1, 16,
                                                GGML_TYPE_Q8_0>
      iq4_nl_16x1_q8_0;
  static const ggml::cpu::repack::tensor_traits<block_q8_0, 1, 16,
                                                GGML_TYPE_Q8_0>
      q8_0_16x1_q8_0;
  static const ggml::cpu::repack::tensor_traits<block_q2_K, 1, 16,
                                                GGML_TYPE_Q8_K>
      q2_K_16x1_q8_K;
#endif
...
static void ggml_backend_cpu_repack_buffer_set_tensor(
    ggml_backend_buffer_t buffer, struct ggml_tensor* tensor, const void* data,
...
static const char* ggml_backend_cpu_repack_buffer_type_get_name(
...
static size_t ggml_backend_cpu_repack_buffer_type_get_alignment(
    ggml_backend_buffer_type_t buft) {
...
class extra_buffer_type : ggml::cpu::extra_buffer_type {
  bool supports_op(ggml_backend_dev_t, const struct ggml_tensor* op) override {
    if (op->op == GGML_OP_MUL_MAT && op->src[0]->buffer &&
      ...
      if (op->src[1]->buffer &&
          !ggml_backend_buft_is_host(op->src[1]->buffer->buft)) {
        return false;
      }
  ...
  ggml::cpu::tensor_traits* get_tensor_traits(
      const struct ggml_tensor* op) override {
