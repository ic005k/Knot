...
bool QuaZipPrivate::goToFirstUnmappedFile() {
  ...
  if (mode != QuaZip::mdUnzip) {
    qWarning(
        "QuaZipPrivate::goToNextUnmappedFile(): ZIP is not open in mdUnzip "
        "mode");
    return false;
  }
  ...
  switch (mode) {
    case mdUnzip:
      ...
      if (ioDevice->isSequential()) {
        ...
        if (!p->zipName.isEmpty()) delete ioDevice;
        qWarning(
            "QuaZip::open(): only mdCreate can be used with sequential "
            "devices");
        return false;
      }
      p->mode = mode;
    ...
    case mdAdd:
      if (ioApi == nullptr) {
        ...
        if (p->utf8) flags |= ZIP_ENCODING_UTF8;
...
QString QuaZip::getComment() const {
  ...
  fakeThis->p->zipError = UNZ_OK;
  unsigned flags = 0;
  return (unzGetFileFlags(p->unzFile_f, &flags) == UNZ_OK) &&
                 (flags & UNZ_ENCODING_UTF8)
...
bool QuaZip::getCurrentFileInfo(QuaZipFileInfo64 *info) const {
  ...
  if ((fakeThis->p->zipError = unzGetCurrentFileInfo64(
           p->unzFile_f, &info_z, nullptr, 0, nullptr, 0, nullptr, 0)) !=
      UNZ_OK)
    return false;
  ...
  if ((fakeThis->p->zipError =
           unzGetCurrentFileInfo64(p->unzFile_f, nullptr, fileName.data(),
                                   fileName.size(), extra.data(), extra.size(),
  ...
  info->name = (info->flags & UNZ_ENCODING_UTF8)
                   ? QString::fromUtf8(fileName)
                   : p->fileNameCodec->toUnicode(fileName);
  info->comment = (info->flags & UNZ_ENCODING_UTF8)
                      ? QString::fromUtf8(comment)
                      : p->commentCodec->toUnicode(comment);
  info->extra = extra;
  info->dateTime =
...
QString QuaZip::getCurrentFileName() const {
  ...
  if ((fakeThis->p->zipError = unzGetCurrentFileInfo64(
           p->unzFile_f, &file_info, fileName.data(), fileName.size(), nullptr,
           0, nullptr, 0)) != UNZ_OK)
    return QString();
...
void QuaZip::setOsCode(uint osCode) { p->osCode = osCode; }

uint QuaZip::getOsCode() const { return p->osCode; }
...
uint QuaZip::getDefaultOsCode() { return QuaZipPrivate::defaultOsCode; }

void QuaZip::setZip64Enabled(bool zip64) { p->zip64 = zip64; }

bool QuaZip::isZip64Enabled() const { return p->zip64; }

void QuaZip::setUtf8Enabled(bool utf8) { p->utf8 = utf8; }

bool QuaZip::isUtf8Enabled() const { return p->utf8; }
