...
Flickable {
    ...
    property real maxRingRadius: Math.min(centerX, centerY) - Math.max(nodeWidth, nodeHeight) / 2 - 60 // 增加边缘安全距离
        ...
        onPaint: {
            ...
            references.forEach(node => {
                if (node.x !== undefined && node.y !== undefined) {
                    ...
                    const startIntersect = calculateRectLineIntersection(
                        centerNodeX, centerNodeY, targetX, targetY,
                        centerNodeX - centerNodeW/2, centerNodeY - centerNodeH/2, centerNodeW, centerNodeH
                    );
                    // 计算目标节点到中心节点中心的交点
                    const endIntersect = calculateRectLineIntersection(
                        targetX, targetY, centerNodeX, centerNodeY,
                    ...
                    if (startIntersect && endIntersect) {
                        drawCurvedArrow(
                            ctx,
                            startIntersect.x, startIntersect.y,
            ...
            referencedBy.forEach(node => {
                 if (node.x !== undefined && node.y !== undefined) {
                    ...
                    // 计算源节点到中心节点中心的交点
                    const startIntersect = calculateRectLineIntersection(
                        sourceX, sourceY, centerNodeX, centerNodeY,
                        node.x, node.y, nodeWidth, nodeHeight
                    );
                    // 计算中心节点到源节点中心的交点
                    const endIntersect = calculateRectLineIntersection(
                        centerNodeX, centerNodeY, sourceX, sourceY,
                        centerNodeX - centerNodeW/2, centerNodeY - centerNodeH/2, centerNodeW, centerNodeH
                    );
    ...
    function calculateRectLineIntersection(px, py, rx, ry, rectX, rectY, rectW, rectH) {
        ...
        if (dx !== 0) {
            ...
            if (t >= 0) {
                ...
                if (iy >= rectY && iy <= rectY + rectH) {
                     if (t < tMin) { tMin = t; intersectX = rectX; intersectY = iy; }
                }
            }
        }
        ...
        if (dx !== 0) {
            ...
            if (t >= 0) {
                const iy = py + t * dy;
                if (iy >= rectY && iy <= rectY + rectH) {
                     if (t < tMin) { tMin = t; intersectX = rectX + rectW; intersectY = iy; }
                }
            }
        }
        ...
        if (dy !== 0) {
            ...
            if (t >= 0) {
                ...
                if (ix >= rectX && ix <= rectX + rectW) {
                     if (t < tMin) { tMin = t; intersectX = ix; intersectY = rectY; }
                }
            }
        }
        ...
        if (dy !== 0) {
            ...
            if (t >= 0) {
                const ix = px + t * dx;
                if (ix >= rectX && ix <= rectX + rectW) {
                     if (t < tMin) { tMin = t; intersectX = ix; intersectY = rectY + rectH; }
                }
            }
        }
    ...
    function calculateNodePositions() {
        ...
        // --- 优化：动态调整环半径以避免节点重叠 ---
        const nodeArcLengthEstimate = nodeWidth + 35; // 增加间距
        ...
        references.forEach((node) => {
            ...
            createNode(node, x, y, "#42a5f5", "#1976d2"); // 蓝色
            index++;
        });
        ...
        referencedBy.forEach((node) => {
            ...
            const x = flickable.centerX + currentRingRadius * Math.cos(angle) - nodeWidth/2;
            const y = flickable.centerY + currentRingRadius * Math.sin(angle) - nodeHeight/2;
            createNode(node, x, y, "#ba68c8", "#6a1b9a"); // 紫色
            index++;
        });
    ...
    function clearNodes() {
         ...
         references.forEach(n => { delete n.x; delete n.y; });
         referencedBy.forEach(n => { delete n.x; delete n.y; });
    }
