...
/*void NotesList::initNoteGraphView() {
  // 1. 注册逻辑保持不动（前提：注册全局/在构造QQuickWidget前执行）
  registerNoteGraphTypes();
  initializeNoteGraph();
  ...
  QTimer::singleShot(0, this, [this]() {
    ...
    // 强制刷新QQuickWidget渲染，Android专用，触发上下文重求值
#ifdef Q_OS_ANDROID
    mui->qwNoteGraphView->update();
    if (mui->qwNoteGraphView->quickWindow()) {
      mui->qwNoteGraphView->quickWindow()->requestUpdate();
    }
#endif
